...
# Saved reports específicos conforme fluxo gravado no Playwright:
# 1) Volnei Pereira
# 2) Demais líderes
RELATORIOS = [
    {"id": "80051142089712073", "nome": "volnei_pereira"},
    {"id": "81391193639005695", "nome": "demais_lideres"},
]
...
ENVIAR_GITHUB = os.getenv("GTN_ENVIAR_GITHUB", "S").strip().upper() not in {"N", "NAO", "NÃO", "NO", "FALSE", "0"}
REMOVER_DUPLICADAS_EXATAS = os.getenv("GTN_REMOVER_DUPLICADAS_EXATAS", "S").strip().upper() not in {"N", "NAO", "NÃO", "NO", "FALSE", "0"}
...
def tentar_abrir_gestao_testes(page: Page) -> bool:
    candidatos = [
        page.get_by_role("treeitem", name="Gestão de Testes"),
        page.get_by_role("button", name="Gestão de Testes"),
        page.get_by_role("link", name="Gestão de Testes"),
        page.get_by_text("Gestão de Testes", exact=True),
        page.locator("a:has-text('Gestão de Testes')"),
        page.locator("span:has-text('Gestão de Testes')"),
        page.locator("button:has-text('Gestão de Testes')"),
    ]
    ...
    for i, candidato in enumerate(candidatos, start=1):
        try:
            alvo = candidato.first
            if alvo.is_visible(timeout=2000):
                log(f"📁 Abrindo/expandindo 'Gestão de Testes' com seletor alternativo #{i}...")
                alvo.scroll_into_view_if_needed()
...
def obter_session_atual(page: Page) -> str:
    try:
        qs = parse_qs(urlparse(page.url).query)
        return (qs.get("session") or [""])[0]
    except Exception:
        return ""


def abrir_execucao_testes_por_url_direta(page: Page) -> bool:
    session = obter_session_atual(page)
    if not session:
        log("⚠️ Não consegui obter session atual para fallback por URL direta.")
        return False

    url_direta = (
        "https://gtn.ninecon.com.br/ords/r/gtn/gtn/"
        "execu%C3%A7%C3%A3o-dos-testes?request=.&clear=NO&"
        f"session={session}"
    )

    try:
        log("🧭 Fallback: abrindo Execução de Testes por URL direta com a sessão atual...")
        page.goto(url_direta, wait_until="domcontentloaded", timeout=DEFAULT_TIMEOUT_MS)
        aguardar_estabilidade(page, "abertura direta da Execução de Testes")
        if tela_execucao_testes_ativa(page):
            log(f"✅ Execução de Testes aberta por URL direta: {page.url}")
            return True
    except Exception as e:
        log(f"⚠️ Fallback por URL direta falhou: {e}")
...
def abrir_execucao_testes(page: Page) -> None:
    ...
    if not menu_aberto:
        ...
        log("ℹ️ Menu não abriu. Tentando URL direta com a sessão atual.")
        if abrir_execucao_testes_por_url_direta(page):
            return
        salvar_debug(page, "sem_menu_no_estado_atual")
        raise RuntimeError(
            "Não encontrei o botão/menu de navegação principal e o fallback por URL direta também falhou."
        )

    tentar_expandir_arvore(page)

    if not tentar_abrir_execucao_testes_pelo_menu(page):
        log("ℹ️ 'Execução de Testes' ainda não apareceu. Vou abrir/expandir 'Gestão de Testes' e tentar novamente.")
        tentar_abrir_gestao_testes(page)
        tentar_expandir_arvore(page)
        page.wait_for_timeout(1200)

    if not tentar_abrir_execucao_testes_pelo_menu(page):
        listar_botoes_visiveis(page)
        log("ℹ️ Menu não expôs 'Execução de Testes'. Tentando URL direta com a sessão atual.")
        if not abrir_execucao_testes_por_url_direta(page):
            salvar_debug(page, "menu_sem_execucao_testes")
            raise RuntimeError(
                "Não encontrei a opção 'Execução de Testes' no menu lateral e o fallback por URL direta também falhou."
            )

    if not tela_execucao_testes_ativa(page):
        log("ℹ️ A tela ainda não confirmou. Tentando URL direta antes de falhar.")
        if not abrir_execucao_testes_por_url_direta(page):
...
def selecionar_relatorio(page: Page, relatorio_id: str, relatorio_nome: str) -> None:
    ...
    assinatura_anterior = obter_assinatura_grade(page)
    aplicar_select_saved_report(page, relatorio_id)
    assinatura_atual = aguardar_grade_mudar(page, assinatura_anterior, relatorio_nome)
    ...
    if assinatura_atual == assinatura_anterior:
        log(
            "⚠️ A assinatura da grade não mudou após selecionar o relatório. "
            "Vou continuar, mas isso pode indicar cache/refresh pendente do APEX."
        )
...
def consolidar_csvs(arquivos_csv: list[Path]) -> Path:
    ...
    destino = DASHBOARD_DIR / "Cenarios_Consolidados_atualizado.csv"
    total_linhas = 0
        ...
        for arquivo in arquivos_csv:
            ...
            with abrir_csv_com_encoding_flexivel(arquivo) as entrada:
                ...
                for linha in reader:
                    ...
                    linha_limpa = [str(col).strip() for col in linha]
                    chave_linha = tuple(linha_limpa)

                    if REMOVER_DUPLICADAS_EXATAS and chave_linha in linhas_vistas:
                        duplicadas_ignoradas += 1
                        continue

                    linhas_vistas.add(chave_linha)
                    writer.writerow(linha_limpa + [gerado_em])
                    linhas_arquivo += 1
                    total_linhas += 1
    ...
    log(f"📊 Total de linhas no consolidado: {total_linhas}")
    if REMOVER_DUPLICADAS_EXATAS:
        log(f"🧼 Duplicidades exatas ignoradas: {duplicadas_ignoradas}")
    return destino


def rodar_git(args: list[str], cwd: Path) -> str:
    ...
    if saida:
        log(f"git {' '.join(args)} -> {saida}")
    if erro:
        log(f"git {' '.join(args)} stderr -> {erro}")
    return saida


def commitar_e_enviar_arquivo(repo_dir: Path, arquivo: Path) -> None:
    if not ENVIAR_GITHUB:
        log("ℹ️ Envio ao GitHub desativado por GTN_ENVIAR_GITHUB=N.")
        return

    if not arquivo.exists():
        raise RuntimeError(f"Arquivo consolidado não encontrado para envio ao GitHub: {arquivo}")
    ...
    if not status.strip():
        log("ℹ️ Nenhuma alteração detectada no consolidado. Nada para commitar.")
        return

    mensagem = f"Atualiza cenários consolidados GTN em {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
    log(f"📝 Criando commit: {mensagem}")
    rodar_git(["commit", "-m", mensagem], repo_dir)

    log("🚀 Enviando consolidado para o GitHub...")
...
def executar_fluxo() -> None:
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            commitar_e_enviar_arquivo(BASE_DIR, arquivo_consolidado)
...
def executar() -> None:
    ...
    try:
        ...
        log("==================================================")
        log("🚀 Iniciando execução GTN - Volnei + demais líderes")
        log("==================================================")
