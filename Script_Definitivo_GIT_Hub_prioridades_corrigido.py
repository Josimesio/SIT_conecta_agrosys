...
# Relatório neutro/base usado para "destravar" o refresh do APEX antes de aplicar o relatório real.
# Veio do comando gravado:
# page.locator("#R35932200234408468_saved_reports").select_option("36017690830433903")
RELATORIO_BASE_REFRESH_ID = "36017690830433903"
RELATORIO_POS_MODAL_ID = RELATORIO_BASE_REFRESH_ID

# Relatórios reais que serão baixados, conforme comandos Playwright gravados.
RELATORIOS = [
    {"id": "81391193639005695", "nome": "relatorio_81391193639005695"},
    {"id": "80051142089712073", "nome": "relatorio_80051142089712073"},
]
...
def aguardar_tela_execucao_testes_renderizada(page: Page) -> None:
    """Aguarda a página de Execução de Testes renderizar de verdade.

    Em Oracle APEX, a URL pode mudar antes da região Interactive Report terminar
    de carregar. O sinal confiável aqui é o select de relatórios salvos da região
    #R35932200234408468 ficar visível.
    """
    log("⏳ Aguardando renderização completa da tela Execução de Testes...")

    try:
        page.wait_for_url(
            lambda url: "execu" in url.lower() and "test" in url.lower(),
            timeout=120000,
        )
    except Exception:
        log("⚠️ URL não confirmou claramente Execução de Testes. Vou validar pelo componente da tela.")

    aguardar_estabilidade(page, "entrada em Execução de Testes")
    aguardar_processamento_apex(page, "carregamento inicial da Execução de Testes")

    page.locator("#R35932200234408468_saved_reports").wait_for(
        state="visible",
        timeout=120000,
    )

    aguardar_processamento_apex(page, "renderização completa da grade Execução de Testes")
    aguardar_estabilidade(page, "pós-renderização da Execução de Testes")
    log("✅ Tela Execução de Testes renderizada e pronta para uso.")


def tentar_expandir_arvore(page: Page) -> None:
    """Expande a árvore do menu sem clicar diretamente no texto do módulo.

    O texto 'Gestão de Testes' pode navegar para uma página intermediária.
    Por isso, primeiro tentamos clicar no toggle real do nó e, se necessário,
    expandimos todos os toggles visíveis.
    """
    try:
        gestao = page.locator("li:has-text('Gestão de Testes')").first
        toggle = gestao.locator(".a-TreeView-toggle").first
        if toggle.is_visible(timeout=3000):
            log("🌲 Expandindo grupo 'Gestão de Testes' pelo toggle real...")
            toggle.scroll_into_view_if_needed()
            toggle.click(timeout=5000, force=True)
            page.wait_for_timeout(1200)
    except Exception:
        pass

    try:
        toggles = page.locator(".a-TreeView-toggle:visible")
        qtd = toggles.count()
        log(f"🌲 Toggles visíveis encontrados na árvore: {qtd}")

        for i in range(qtd):
            try:
                toggle = toggles.nth(i)
                toggle.scroll_into_view_if_needed()
                toggle.click(timeout=3000, force=True)
                page.wait_for_timeout(500)
            except Exception:
                continue
    except Exception:
    ...
    for i, candidato in enumerate(candidatos, start=1):
        try:
            ...
            if alvo.is_visible(timeout=2500):
                ...
                aguardar_tela_execucao_testes_renderizada(page)
                return True
        except Exception as e:
            log(f"⚠️ Seletor alternativo #{i} falhou para Execução de Testes: {e}")
            continue
...
def selecionar_relatorio(page: Page, relatorio_id: str, relatorio_nome: str) -> None:
    ...
    for tentativa in range(1, MAX_TENTATIVAS_APLICAR_RELATORIO + 1):
        log(f"🔁 Aplicando relatório {relatorio_nome} - tentativa {tentativa}/{MAX_TENTATIVAS_APLICAR_RELATORIO}")

        assinatura_antes = obter_assinatura_grade(page)

        # Primeiro aplica o relatório base/neutro gravado no codegen.
        # Isso força o APEX a trocar o estado da região antes do relatório real.
        aplicar_select_saved_report(page, RELATORIO_BASE_REFRESH_ID)
        aguardar_processamento_apex(page, f"pré-seleção base {RELATORIO_BASE_REFRESH_ID}")
        aguardar_estabilidade(page, f"pré-seleção base {RELATORIO_BASE_REFRESH_ID}")
        page.wait_for_timeout(5000)

        assinatura_base = obter_assinatura_grade(page)

        # Agora aplica o relatório real, disparando input/change via Playwright + JS fallback.
        aplicar_select_saved_report(page, relatorio_id)
        aguardar_processamento_apex(page, f"seleção do relatório {relatorio_nome}")
        aguardar_estabilidade(page, f"seleção do relatório {relatorio_nome}")

        assinatura_depois = aguardar_grade_mudar(page, assinatura_base or assinatura_antes, relatorio_nome)
        ...
        log(
            f"🧾 Relatório selecionado na tela -> id atual: {valor_atual or '(indisponível)'} "
            f"| nome visível: {texto_atual}"
        )

        if valor_atual == relatorio_id:
            if assinatura_depois and assinatura_depois != assinatura_base:
                log("✅ Grade confirmou alteração após seleção do relatório.")
            else:
                log("⚠️ ID confirmado, mas a assinatura da grade não mudou. Seguindo com cautela.")
            return

        log(
            f"⚠️ Select ainda não confirmou o ID esperado. Esperado: {relatorio_id} | "
            f"Atual: {valor_atual or '(indisponível)'}"
        )
        page.wait_for_timeout(2000)

    raise RuntimeError(
        f"Não consegui aplicar o relatório {relatorio_nome}. "
        f"ID esperado: {relatorio_id}."
    )
...
def normalizar_nome_coluna(nome: str) -> str:
    return (nome or "").strip().lower().replace("í", "i").replace("á", "a").replace("ã", "a").replace("â", "a").replace("é", "e").replace("ê", "e").replace("ó", "o").replace("ô", "o").replace("õ", "o").replace("ú", "u").replace("ç", "c")


def obter_indice_coluna_prioridade(cabecalho: list[str]) -> int:
    for indice, coluna in enumerate(cabecalho):
        nome = normalizar_nome_coluna(coluna)
        if nome == "prioridade" or "prioridade" in nome:
            return indice

    raise RuntimeError(
        "Coluna de prioridade não encontrada no CSV. "
        "Não vou gerar o consolidado sem conseguir aplicar o filtro P1, P2, P3 e P9."
    )


def prioridade_permitida(valor: str) -> bool:
    prioridade = (valor or "").strip().upper().replace(" ", "")
    return prioridade in {"P1", "P2", "P3", "P9"}
    ...
    with open(destino, "w", newline="", encoding="utf-8-sig") as saida:
        ...
        for arquivo in arquivos_csv:
            ...
            with abrir_csv_com_encoding_flexivel(arquivo) as entrada:
                ...
                indice_prioridade = obter_indice_coluna_prioridade(cabecalho_atual)
                linhas_arquivo = 0
                ...
                for linha in reader:
                    ...
                    prioridade = linha[indice_prioridade] if indice_prioridade < len(linha) else ""
                    if not prioridade_permitida(prioridade):
                ...
                log(
                    ...
                    f"com prioridade P1/P2/P3/P9. Ignoradas por prioridade: {linhas_ignoradas_prioridade}."
                )
